...
class CallManager: NSObject {
  ...
  func acceptCall(call: Call) {
    let endCallAction = CXAnswerCallAction(call: call.uuid)
    let callTransaction = CXTransaction()
    callTransaction.addAction(endCallAction)
    //requestCall
    self.requestCall(callTransaction, action: "acceptCall")
  }
  ...
  func endActiveCall() {
    ...
    for call in calls {
      ...
      let callTransaction = CXTransaction()
      callTransaction.addAction(endCallAction)
      self.requestCall(callTransaction, action: "endActiveCall")
    }
  }
  ...
  private func requestCall(
    ...
    callController.request(transaction) { error in
      if let error = error {
        ...
        } else if action == "endCall" || action == "endCallAlls" || action == "endActiveCall" {
